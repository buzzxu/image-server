...
func (c *httpClient) upload(url string, filename string, fileReader io.Reader, mtype string, token string) (respBody []byte, statusCode int, err error) {
	r, w := io.Pipe()
...
func (c *httpClient) delete(url string, token string) (statusCode int, err error) {
	req, err := http.NewRequest(http.MethodDelete, url, nil)
	if err != nil {
		return
	}
	req.Header.Set(HeadAuthorization, token)
